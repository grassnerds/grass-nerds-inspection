...
// ─── Delete an inspection and its related data ───
export async function deleteInspection(inspectionId) {
  // Delete in order: resolutions → responses → inspection
  await supabase.from('issue_resolutions').delete().eq('inspection_id', inspectionId);
  await supabase.from('inspection_responses').delete().eq('inspection_id', inspectionId);
